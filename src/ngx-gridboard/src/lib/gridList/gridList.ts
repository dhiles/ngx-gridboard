const getGridCol = function (lanes) {
  const gridCol = [];
...
export class GridListHelper {
  ...
  static cloneItems(items, _items?) {
    if (_items === undefined) {
      _items = [];
    }
...
export class GridList {
  ...
  toString() {
    ...
    for (let i = 0; i < this._options.lanes; i++) {
      output += '\n' + this._padNumber(i, ' ') + '|';
  ...
  findPositionForItem(item, start, fixedRow?) {
    ...
    for (let x = start.x; x < this.grid.length; x++) {
      if (fixedRow !== undefined) {
        ...
        for (let y = start.y; y < this._options.lanes; y++) {
          position = [x, y];
  ...
  getChangedItems(initialItems, idAttribute) {
    ...
    for (let i = 0; i < initialItems.length; i++) {
      const item = this._getItemByAttribute(idAttribute,
  ...
  _itemFitsAtPosition(item, newPosition) {
    ...
    // item.
    for (let x = newPosition[0]; x < newPosition[0] + position.w; x++) {
      ...
      for (let y = newPosition[1]; y < newPosition[1] + position.h; y++) {
  ...
  _ensureColumns(N) {
    ...
    for (let i = 0; i < N; i++) {
      if (!this.grid[i]) {
        this.grid.push(getGridCol(this._options.lanes));
      }
    }
  }
  ...
  _tryToResolveCollisionsLocally(item) {
    ...
    const _gridList = new GridList([], this._options);
    let leftOfItem,
      rightOfItem,
      aboveOfItem,
      belowOfItem;

      GridListHelper.cloneItems(this.items, _gridList.items);
    ...
    // original one
    GridListHelper.cloneItems(_gridList.items, this.items);
    this.generateGrid();
    return true;
  }
  ...
  _pullItemsToLeft(fixedItem?) {
    ...
    for (let i = 0; i < this.items.length; i++) {
      ...
      const x = this._findLeftMostPositionForItem(item),
